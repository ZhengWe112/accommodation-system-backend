...
/*==============================================================*/
drop database if exists accommodation_management_system;
...
(
    ...
    role_id              smallint default 3 comment '外键 关联到表role 这个宿管的角色 默认都是3 一般不会改变',
...
(
    id                   bigint auto_increment not null comment '主键',
    role_id              smallint default 2 comment '外键 关联到表role 维修管理员的角色 默认都是2 一般不会改变',
    fullname             varchar(15) comment '全名',
    job_id               char(13) comment '工号 用于登录 固定为13位 虽然也是以id结尾 但它不是外键',
    password             varchar(20) comment '密码',
    create_time          datetime default current_timestamp,
...
insert into role(role_id, role_name) VALUES
                                         (1, '系统管理员'), (2, '维修管理员'), (3, '宿舍管理员'), (4, '分管领导'), (5, '教师'), (6, '学生');
...
(
    ...
    state                smallint default 0 comment '状态 0未送审分管领导 1已送审分管领导 2已公布成绩',
    create_time          datetime default current_timestamp,
    update_time          datetime default current_timestamp on update current_timestamp,
    is_deleted           bool default 0,
    primary key (id)
